...
C-------------------- FIX FOR CHARGE CORRECTIONS (only for 1497)
      DATA ICHRG/
     & -1, 2, -1, 2, -1/ 
C-------------------------------------------------------------------
...
C---READ AN EVENT
C      IF(NQQ.GE.MQQ)CALL HWWARN('UPEVNT',201)
...
C---LOAD BOSON ID
         ...
         ELSEIF (JPR.EQ.14) THEN
            ...
            DO I=1,NUP-1
               ID=IDUP(I)
C----------- SANITY CHECK DUE TO A HACK (FST, 21.3.2008)
               IF((ABS(ID).GT.5).AND.(ID.NE.21)) THEN
                  WRITE(*,*) 'FATAL ERROR IN UPEVNT'
                  STOP
...
C-------- SAFTY CHECK FOR CMSSW, WE REMOVE ee collisions
          WRITE(*,*) 'EE COLLSIONS NOT SUPPORTED FOR CMSSW'
          STOP
